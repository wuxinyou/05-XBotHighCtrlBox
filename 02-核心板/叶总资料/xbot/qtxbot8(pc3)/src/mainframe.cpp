...
#define ABS_value(x) (((x)<0)?(-(x)):(x))
	...
	QLibrary mylib1("devwdm.dll");

	typedef long (__stdcall *Fun)(int,HWND,int,int); //定义函数指针，以备调用
	typedef long (__stdcall *Fun1)(int); //定义函数指针，以备调用
	typedef long (__stdcall *Fun2)(int,int,int,int,int); //定义函数指针，以备调用
	typedef long (__stdcall *Fun3)(int, char*); //定义函数指针，以备调用
	typedef long (__stdcall *Fun4)(int, char*); //定义函数指针，以备调用
	typedef long (__stdcall *Fun5)(int); //定义函数指针，以备调用
	typedef long (__stdcall *Fun6)(int); //定义函数指针，以备调用
	typedef long (__stdcall *Fun7)(int); //定义函数指针，以备调用
	typedef long (__stdcall *Fun8)(char *); //定义函数指针，以备调用
	
	//typedef long ( *Fun)(int,HWND,int,int); //定义函数指针，以备调用
	//typedef long ( *Fun1)(int); //定义函数指针，以备调用
	//typedef long ( *Fun2)(int,int,int,int,int); //定义函数指针，以备调用
	//typedef long ( *Fun3)(int, char*); //定义函数指针，以备调用
	//typedef long ( *Fun4)(int, char*); //定义函数指针，以备调用
	//typedef long ( *Fun5)(int); //定义函数指针，以备调用
	//typedef long ( *Fun6)(int); //定义函数指针，以备调用
	//typedef long ( *Fun7)(int); //定义函数指针，以备调用
	//typedef long ( *Fun8)(char *); //定义函数指针，以备调用
	//
	Fun initv=(Fun)mylib1.resolve("devwdm_InitCard");
	Fun1 opendev=(Fun1)mylib1.resolve("devwdm_StartVideoPreview");
	Fun2 dispos=(Fun2)mylib1.resolve("devwdm_SetDisplayPos");
	Fun3 saveBMP=(Fun3)mylib1.resolve("devwdm_SaveBmpFile");
	Fun4 startMP4=(Fun4)mylib1.resolve("devwdm_StartCaptureMp4");
	Fun5 stopMP4=(Fun5)mylib1.resolve("devwdm_stopCaptureMp4");
	Fun6 Pause=(Fun6)mylib1.resolve("devwdm_PreviewPause");
	Fun7 Resume=(Fun7)mylib1.resolve("devwdm_PreviewResume");
	Fun8 getpicture=(Fun8)mylib1.resolve("devwdm_GetImageBuffer");
using namespace std; 
	...
	: QDialog(parent, f)
{
	...
	mywid_2d->setGeometry(234*w,135*h,332*w,164*h);
	/********************************************/
	...
	mypitch->setGeometry(647*w,434*h,50*w,47*h);
	...
	initv(0, this->winId(),1200,900);
	opendev(0);
	dispos(0,56*w,0*h,688*w,432*h);
	//dispos(0,0*w,0*h,800*w,600*h);
...
void Mainframe::disposejoy(QByteArray joy)
{
	if(((unsigned char)joy.at(5)&0x02)==0x02&&(unsigned char)joy.at(0)==0&&(unsigned char)joy.at(1)==0&&(unsigned char)joy.at(2)==0&&(unsigned char)joy.at(3)==0&&(unsigned char)joy.at(4)==0)//改变模式
	{
		...
		if(modelflag==0)
		{
			...
			myselect->setValue_2(0x00);
			myselect->setValue_1((int)0x00);
		}			
	}
	else
	{
...
void Mainframe::paintEvent(QPaintEvent *)
{
	...
	switch(modelflag)
	{
		...
		case 0:
			if(!pix.load("images/back1.jpg",0,Qt::AvoidDither|Qt::ThresholdDither|Qt::ThresholdAlphaDither))
			{
				QMessageBox::information(this, "", "failed load file");
			}
			break;			
		case 1:
			if(!pix.load("images/back2.jpg",0,Qt::AvoidDither|Qt::ThresholdDither|Qt::ThresholdAlphaDither))
			{
				QMessageBox::information(this, "", "failed load file");
			}
			break;			
		case 2:
			if(!pix.load("images/back3.jpg",0,Qt::AvoidDither|Qt::ThresholdDither|Qt::ThresholdAlphaDither))
			{
				QMessageBox::information(this, "", "failed load file");
			}
			break; 			
