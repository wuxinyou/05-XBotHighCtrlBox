...
void Mainframe::initComponent()
{
	...
	but_M2_A7->setPressIco(":/images/yuliu_2.png");
	...
	but_M2_A7->setReleaseIco(":/images/yuliu_1.png");
	...
	mywid->setGeometry(104*w,453*h,259*w,143*h);
...
void Mainframe::initConnect()
{
	...
	connect(sizeCtr3d, SIGNAL(valueChanged(double)), mywid, SLOT( updatesize(double)));
...
void Mainframe::disposejoy(QByteArray joy)
{
	...
	if(key_2!=backjoy[2])//key_e 
	{
		...
		{	
			if((commanddata[3]&0x07)==0x06)//如果处在三维控制模式下
			{
		...
		else//shift键没按下
		{
			...
			else
			{
				...
				{
					mywid->setGeometry(104*w,653*h,259*w,143*h);
...
{	
	...
	switch(a)
	{
		...
		case 0x81:
			...
			{
				backdata[7]=str.at(3);
				mywid->dabiPitch(backdata[7]*2-180);
		...
		case 0x82:
			if(backdata[12]!=str.at(3))//小臂俯仰角度
			{
				...
				mywid->xiaobiPitch(backdata[12]*2+192);
		...
		case 0x83:
			if(backdata[17]!=str.at(3))//腕关节角度
			{
				...
				mywid->shouwanPitch(backdata[17]*2-4);
