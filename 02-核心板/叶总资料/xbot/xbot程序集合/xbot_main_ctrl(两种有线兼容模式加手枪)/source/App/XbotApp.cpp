...
//最大参考速度宏
#define REFER_SPEED_MAX_ARM0		7000	//支臂
#define REFER_SPEED_MAX_ARM1		8000	//大臂旋转
#define REFER_SPEED_MAX_ARM2		5000	//大臂俯仰
#define REFER_SPEED_MAX_ARM3		5000	//小臂俯仰
#define REFER_SPEED_MAX_ARM4		5000	//手腕俯仰 
#define REFER_SPEED_MAX_ARM5		12000 //手爪旋转
#define REFER_SPEED_MAX_ARM6		9000 	//云台俯仰
#define REFER_SPEED_MAX_ARM7		2000 	//云台旋转
#define REFER_SPEED_MAX_ARM8		9000 	//手爪加持
//最小参考角度宏
...
XbotApp::XbotApp()
{
	...
	reGunSendOk=true;							//武器发射发送完成标志 默认完成 
...
/**
  * 功  能：发送武器发射命令
  * 参  数：无
  * 返回值：false,发送未完成；true，发送完成
  * 
  */
bool XbotApp::sendWeaponCommand()
{
	static uint8_t index=0x00;
	static bool sendLimitFlag=true;
	if(!reWeaponSendOk)//发送命令开启
	{
	...
	else 
	{
		if(index<0x0a)
		{
			if(!sendLimitFlag)
			{
				OtherDevice->sendWeaponControlCommand();//补满十次
				index++;
				return false;
			}
		}
...
/**
  ...
  * 
  */
bool XbotApp::sendGunCommand()
{
	static uint8_t index=0x00;
	static bool sendLimitFlag=true;
	if(!reGunSendOk)//发送命令开启
	{
		OtherDevice->sendGunControlCommand();
		if(index<0x02)
			index++;
		sendLimitFlag=false;
		return false;
	...
	else 
	{
		if(index<0x02)
		{
			if(!sendLimitFlag)
			{
				OtherDevice->sendGunControlCommand();//补满十次
				index++;
				return false;
			}
		}
...
void XbotApp::disposeTerminalData()
{
		...
		&&temData[6]==0xEE&&temData[7]==0xFF&&temData[8]==0x11&&temData[9]==0x22&&temData[10]==0x33&&temData[11]==0x44)
	{
		...
		&&temData[6]==0xEE&&temData[7]==0xFF&&temData[8]==0x55&&temData[9]==0x66&&temData[10]==0x77&&temData[11]==0x88)
	{
		resetGun();
	}
	else	//正常指令
...
//行驶模式和抓取模式的判定
	...
	{
		fingerMediateM->setErrorFlag_1(setFingerMediateSpeed(-((int16_t)((fBTerminalData[8]&0x70)>>0x04)*(REFER_SPEED_MAX_ARM8/7))));
	}
	else
	{
		fingerMediateM->setErrorFlag_1(setFingerMediateSpeed((int16_t)((fBTerminalData[8]&0x70)>>0x04)*(REFER_SPEED_MAX_ARM8/7)));
...
// 						temRightSpeed=(fBTerminalData[2]*LandRMoterMaxSpeed)/150;
	...
	switch((int)workId)
	{
		...
		if(signalType==WIRE_WORK_MODE)//线缆盘自动收放线
		{
			...
			{
				bigArmRotateM->setErrorFlag_1(setBigArmRotateSpeed((int16_t)((fBTerminalData[4]&0x70)>>0x04)*(REFER_SPEED_MAX_ARM1/7)));
			}
			else
			{
				bigArmRotateM->setErrorFlag_1(setBigArmRotateSpeed(-((int16_t)((fBTerminalData[4]&0x70)>>0x04)*(REFER_SPEED_MAX_ARM1/7))));
			...
			{
				bigArmPitchM->setErrorFlag_1(setBigArmPitchSpeed((int16_t)((fBTerminalData[4]&0x07)*(REFER_SPEED_MAX_ARM2/7))));
			}
			else
			{
				bigArmPitchM->setErrorFlag_1(setBigArmPitchSpeed(-((int16_t)((fBTerminalData[4]&0x07)*(REFER_SPEED_MAX_ARM2/7)))));
			...
			{
				forearmPitchM->setErrorFlag_1(setForearmPitchSpeed(-((int16_t)((fBTerminalData[5]&0x70)>>0x04)*(REFER_SPEED_MAX_ARM3/7))));
			}
			else
			{
				forearmPitchM->setErrorFlag_1(setForearmPitchSpeed((int16_t)((fBTerminalData[5]&0x70)>>0x04)*(REFER_SPEED_MAX_ARM3/7)));
			...
			{
				wristPitchM->setErrorFlag_1(setWristPitchSpeed(-((int16_t)((fBTerminalData[5]&0x07)*(REFER_SPEED_MAX_ARM4/7)))));
			}
			else
			{
				wristPitchM->setErrorFlag_1(setWristPitchSpeed((int16_t)((fBTerminalData[5]&0x07)*(REFER_SPEED_MAX_ARM4/7))));
			...
			{
				wristRotateM->setSpeed((int16_t)((fBTerminalData[6]&0x70)>>0x04)*(REFER_SPEED_MAX_ARM5/7));
			}
			else
			{
				wristRotateM->setSpeed(-((int16_t)((fBTerminalData[6]&0x70)>>0x04)*(REFER_SPEED_MAX_ARM5/7)));
			...
			if(fBTerminalData[7]&0x08)
			{
				...
				temsetPanTiltPitchSpeed=temsetPanTiltPitchSpeed*temsetPanTiltPitchSpeed*(REFER_SPEED_MAX_ARM6/49);
			...
			if(fBTerminalData[7]&0x80)
			{
				...
				temsetPanTiltRotateSpeed=temsetPanTiltRotateSpeed*temsetPanTiltRotateSpeed*(REFER_SPEED_MAX_ARM7/49);
			...
			{
				supportArmM->setErrorFlag_1(setSupportArmSpeed(-((int16_t)((fBTerminalData[6]&0x07)*(REFER_SPEED_MAX_ARM0/7)))));
			}
			else
			{
				supportArmM->setErrorFlag_1(setSupportArmSpeed((int16_t)((fBTerminalData[6]&0x07)*(REFER_SPEED_MAX_ARM0/7))));
...
void XbotApp::fingerRunToNNNModel()
{
	...
	if(temWristRotateA>0)//负方向运动
	{
		...
		{
			wristRotateM->setSpeed(-REFER_SPEED_MAX_ARM5);
		}
...
void XbotApp::XbotArmSetXyzCtrlMode()
{
	...
	if(fBTerminalData[6]&0x80)
	{
		TempSpeed=((int16_t)((fBTerminalData[6]&0x70)>>0x04)*(REFER_SPEED_MAX_ARM5/7));
	}
	else
	{
		TempSpeed=(-((int16_t)((fBTerminalData[6]&0x70)>>0x04)*(REFER_SPEED_MAX_ARM5/7)));
...
double XbotApp::runOnXYZLine(int8_t direction,double *Angle,int16_t speed)				//平行于笛卡尔坐标的X轴运动 闭环控制
{
	...
	if(direction==1)				//y方向运动
	{
		...
		else
		{
			...
			threeArmRunToPos(temA0,temA1,temA2,speed/2);
		}
