...
//	PC4		IN_Fire			开火控制
	...
	else //钥匙按钮打开
	{
		...
		{
			if(CbCtrlData.TestMode == TEST_GUN)
			{
				if(!gunFlaggunFlag)
				{
					gunFlag=true;
					CbCtrlData.FireState = GUN_START;//0x03
				}
			}
			else if(CbCtrlData.TestMode == TEST_FIRE)
			{
...
//			}
	...
	else if(CbCtrlData.FireState == FIRE_PREPARE)//0X01
	{
	...
	else if(CbCtrlData.FireState == FIRE_START)//0X02
	{
		for(count=0;count<CB_TXD_LEN-1;count++)
			CbCtrlData.TxdDataBuf[count] = CB_FIRE_DATA[count];
	}
	else if(CbCtrlData.FireState == GUN_START)//0X03
	{
		for(count=0;count<CB_TXD_LEN-1;count++)
