	...
	: QDialog(parent, f)
{
	...
	datatime.setTime(QTime());   //设置系统时间为0
...
void Mainframe::initComponent()
{
	...
	signalCountTimer =new QTimer(this); 	
	//
	//添加串口
	sendFrame    = new SendFrame;
	//receiveFrame = new ReceiveFrame;
    serialPortThread = new SerialPortThread(NULL,sendFrame);
...
void Mainframe::initConnect()
{
	...
	connect(signalCountTimer,SIGNAL(timeout()),this,SLOT(showSignal()));
	connect(sendTimer, SIGNAL(timeout()), this, SLOT(sendcommand())); 	
	connect(serialPortThread, SIGNAL(robotDataSignal(ReceiveFrame *)), this, SLOT( readyRead(ReceiveFrame *)));
	connect(serialPortThread, SIGNAL(keyDataSignal(ReceiveFrame *)), this, SLOT( disposejoy(ReceiveFrame *)));
...
author:wuxinyou
data  :2017-02-21
note  :同步控制数据到串口线程,定时发送函数，其中sendFrame是和串口线程共享的数据
*****************************************************************************************/
void Mainframe::sendcommand()//定时器溢出响应函数，显示时间和发送命令
{
	...
	for(int i=0;i<13;i++)
	{
		sendFrame->byte[i+2]=commanddata[i];
	}
...
author:wuxinyou
data  :2017-02-21
note  :响应物理按键,收到按键数据后触发该函数，其中receiveFrame是和串口线程共享的数据
*****************************************************************************************/
void Mainframe::disposejoy(ReceiveFrame *receiveFrame)
{
	unsigned char key_0,key_1,key_2,key_3,key_4,key_5,key_6,key_7,key_8,key_9,key_10,key_11;
	unsigned char tem=0;
	key_0=(unsigned char)receiveFrame->keyData.keyA;
	key_1=(unsigned char)receiveFrame->keyData.keyB;
	key_2=(unsigned char)receiveFrame->keyData.keyE;
	key_3=(unsigned char)receiveFrame->keyData.keyF;
	key_4=(unsigned char)receiveFrame->keyData.keyHM;
	key_5=(unsigned char)receiveFrame->keyData.keyIJG;
	key_6=(unsigned char)receiveFrame->keyData.joyStickLeftX;
	key_7=(unsigned char)receiveFrame->keyData.joyStickLeftY;
	key_8=(unsigned char)receiveFrame->keyData.joyStickRightX;
	key_9=(unsigned char)receiveFrame->keyData.joyStickRightY;
	key_10=(unsigned char)receiveFrame->keyData.ctrlBoxPower1;
	key_11=(unsigned char)receiveFrame->keyData.ctrlBoxPower2;
	...
	{
		mybattery_4->setValue(key_10);   //原来是setValue(backjoy[10])
		backjoy[10]=key_10;
	}
	if(key_11!=backjoy[11])//电池2电压
	{
		mybattery_5->setValue(key_11);
		backjoy[11]=key_11;
	}
}
...
author:wuxinyou
data  :2017-02-21
note  :响应机器人的数据,收到机器人数据后触发该函数，其中receiveFrame是和串口线程共享的数据
*****************************************************************************************/
void Mainframe::readyRead(ReceiveFrame *receiveFrame)
{	
	...
	if(backdata[0]!=receiveFrame->robotData.leftMoveSpeed)//左电机速度
	{
		backdata[0]=receiveFrame->robotData.leftMoveSpeed;
		//myspeed->setValue_1((signed int)((signed char)backdata[0])*2); //新版控制器有速度反馈
	}
	if(backdata[1]!=receiveFrame->robotData.rightMoveSpeed)//右电机速度
	{
		backdata[1]=receiveFrame->robotData.rightMoveSpeed;
		//myspeed->setValue(-(signed int)((signed char)backdata[1])*2);
	}
	unsigned char a=(unsigned char)(receiveFrame->robotData.feedIndex);  
	switch(a)
	{
		case 0x80:
			if(backdata[2]!=receiveFrame->robotData.feedData1)//底盘旋转角度
			{
				backdata[2]=receiveFrame->robotData.feedData1;
				// no_use3d mywid->dabiRotate(backdata[2]*2);
				mywid_2d->setValue_7(backdata[2]*2);
			}
			if(backdata[3]!=receiveFrame->robotData.feedData2)//手爪旋转角度
			{
				backdata[3]=receiveFrame->robotData.feedData2;
				// no_use3d mywid->shouwanRotate(-backdata[3]*2);
				mywid_2d->setValue_6(backdata[3]*2);
			}
			if(backdata[4]!=receiveFrame->robotData.feedData3)//手爪抓持宽度
			{
				backdata[4]=receiveFrame->robotData.feedData3;
				// no_use3d mywid->shouzhuaMediate(-backdata[4]*2);
				mywid_2d->setValue_5(backdata[4]*2);
			}
			if(backdata[5]!=receiveFrame->robotData.feedData4)//摆臂模式控制??????????
			{
				backdata[5]=receiveFrame->robotData.feedData4;
			}
			if(backdata[6]!=receiveFrame->robotData.feedData4)//电池电压1
			{
				backdata[6]=receiveFrame->robotData.feedData4;
				mybattery->setValue(backdata[6]*2);
			}															
			break;
		case 0x81:
			if(backdata[7]!=receiveFrame->robotData.feedData1)//大臂俯仰角度
			{
				backdata[7]=receiveFrame->robotData.feedData1;
				// no_use3d mywid->dabiPitch(backdata[7]*2-180);
				mywid_2d->setValue_1(backdata[7]*2);
			}
			if(backdata[8]!=receiveFrame->robotData.feedData2)//云台俯仰角度
			{
				backdata[8]=receiveFrame->robotData.feedData2;
				// no_use3d mywid->yuntaiPitch(backdata[8]*2+180);
				mywid_2d->setValue_4(backdata[8]*2);
			}
			if(backdata[9]!=receiveFrame->robotData.feedData3)//前支臂角度
			{
				backdata[9]=receiveFrame->robotData.feedData3;
				// no_use3d mywid->zhibiPitch(backdata[9]*2);
				mypitch->setValue_1((backdata[9]*2+180)%360);
				mywid_2d->setValue_9(backdata[9]*2);
			}
			if(backdata[10]!=receiveFrame->robotData.feedData4)//机械臂模式??????
			{
				backdata[10]=receiveFrame->robotData.feedData4;
			}
			if(backdata[11]!=receiveFrame->robotData.feedData4)//电池电压2
			{
				backdata[11]=receiveFrame->robotData.feedData4;
				mybattery_1->setValue(backdata[11]);
			}				
			break;
		case 0x82:
			if(backdata[12]!=receiveFrame->robotData.feedData1)//小臂俯仰角度
			{
				backdata[12]=receiveFrame->robotData.feedData1;
				// no_use3d mywid->xiaobiPitch(backdata[12]*2+187);
				mywid_2d->setValue_2(backdata[12]*2);
			}
			if(backdata[13]!=receiveFrame->robotData.feedData2)//云台旋转角度
			{
				backdata[13]=receiveFrame->robotData.feedData2;
				// no_use3d mywid->yuntaiRotate(backdata[13]*2);
				mywid_2d->setValue_8(backdata[13]*2);
			}
			if(backdata[14]!=receiveFrame->robotData.feedData3)//保留
			{
				backdata[14]=receiveFrame->robotData.feedData3;
			
			}
			if(backdata[15]!=receiveFrame->robotData.feedData4)//保留
			{
				backdata[15]=receiveFrame->robotData.feedData4;
				
			}
			if(backdata[16]!=receiveFrame->robotData.feedData4)//电池电压3
			{
				backdata[16]=receiveFrame->robotData.feedData4;
				mybattery_2->setValue(backdata[16]);
			}			
			break;
			//
		case 0x83:
			if(backdata[17]!=receiveFrame->robotData.feedData1)//腕关节角度
			{
				backdata[17]=receiveFrame->robotData.feedData1;
				// no_use3d mywid->shouwanPitch(backdata[17]*2-3);
				mywid_2d->setValue_3(backdata[17]*2);
			}
			if(backdata[18]!=receiveFrame->robotData.feedData2)//摄像头俯仰角度
			{
				backdata[18]=receiveFrame->robotData.feedData2;
				// no_use3d mywid->qianjinxiangjiPitch(backdata[18]*2);
			}
			if(backdata[19]!=receiveFrame->robotData.feedData3)//系统电压
			{
				backdata[19]=receiveFrame->robotData.feedData3;
				
			}
			if(backdata[20]!=receiveFrame->robotData.feedData4)//保留
			{
				backdata[20]=receiveFrame->robotData.feedData4;
				
			}
			if(backdata[21]!=receiveFrame->robotData.feedData4)//电池电压4
			{
				backdata[21]=receiveFrame->robotData.feedData4;
