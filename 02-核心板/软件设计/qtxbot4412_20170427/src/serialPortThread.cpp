...
SerialPortThread::SerialPortThread(ReceiveFrame *recData,SendFrame *sendData)
{
	...
	receiveKeyFrame=new ReceiveFrame;
	receiveRobotFrame=new ReceiveFrame;
...
int SerialPortThread::openSerialPort()
{
        ...
        serialAttr.c_cflag = B19200 | HUPCL | CS8 | CREAD | CLOCAL;
...
bool SerialPortThread::receiveData(char c)
{
	...
	{	
		...
		if(checkSum==tempData[RECEIVE_FRAME_LEN-1])
		{
			...
			if(tempData[1]==0x10)    //收到的是机器人数据
			{
				for(int i=0;i<QUEUESIZE-1;i++)
				{
					receiveRobotFrame->byte[i]=tempData[i];				
				}
				emit robotDataSignal(receiveRobotFrame);
			}
			else if(tempData[1]==0x30)//收到的是按键数据
			{
				for(int i=0;i<QUEUESIZE-1;i++)
				{
					receiveKeyFrame->byte[i]=tempData[i];				
				}
				emit keyDataSignal(receiveKeyFrame);
			}
			return true;
		}
		//校验处理完
